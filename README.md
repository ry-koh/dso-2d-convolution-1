...
when any downstream consumer is not ready.
...
|---|---|---|---|
| `DATA_WIDTH` | positive | 8 | Bits per pixel (any positive integer; 8/16 verified) |
| `KERN_ROWS` | positive | 3 | Kernel height M (any value; 3 and 5 verified) |
| `KERN_COLS` | positive | 3 | Kernel width N (any value; 3 and 5 verified) |
...
All 36 configurations below pass bit-exactly in Vivado xsim (simulation time: 2955 ns,
3 frames of 8×8 pixels each):
...
tb/
  conv2d_tb.vhd       Self-checking testbench (machine-generated by gen_tb.py)
                      Runs all 36 DUT instances in parallel on a shared clock
  vectors/
    c01_input.txt     Stimulus pixels for CFG01
    c01_expected.txt  Expected tap values for CFG01
    ...               (72 files total, c01..c36 × input/expected)

scripts/
  gen_tb.py           Regenerates conv2d_tb.vhd and all 72 vector files.
