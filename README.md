...
tb/
  conv2d_tb.vhd       Self-checking testbench (machine-generated by gen_tb.py — do not hand-edit)
                      324 DUT instances in parallel on a shared clock; 3 back-pressure scenarios;
                      30 stress scenarios with varied s_tvalid/m_tready timing
  visualize.html      Self-contained interactive visualiser (generated by gen_tb.py --html)
                      ...
                      Embeds cycle-accurate traces for all 324 configs + 30 stress scenario overlays
  vectors/
...
scripts/
  gen_tb.py           Single source-of-truth code generator. Produces:
                        • tb/vectors/c001..c324_{input,expected}.txt  (648 vector files)
                        • tb/conv2d_tb.vhd  (VHDL testbench, 324 base + 30 stress DUTs)
                        • tb/visualize.html  (interactive visualiser, with --html flag)
                      Run: python scripts/gen_tb.py [--html]  (from repo root)

CLAUDE.md             Full project log: decisions, bug log, phase status, locked constraints
PROMPTS.md            Prompting guide: how this project was built with Claude, phase by phase
README.md             This file
```
...
**Back-pressure processes** — one process per BP scenario (3 total), deasserts the
appropriate `m_tready` bit at the configured time and reasserts it after the stall period.

**Stress scenario DUTs** — 30 additional DUT instances with varied `s_tvalid` and
`m_tready` patterns (see below). Each stress scenario's checker reads the same expected
file as its base config and compares at every `mvalid AND mready` handshake.

`sim_done` goes true when all 354 `done_flags` bits (324 base + 30 stress) are set,
which stops the clock.

### 5. Stress scenario generation

30 stress scenarios are defined in `STRESS_CONFIGS` as `(kr, kc, mode, flush, lw, fh, sv_h, sv_l, mr_h, mr_l, desc)` tuples. `sv_h/sv_l` is the `s_tvalid` HIGH/LOW period in clock cycles; `mr_h/mr_l` is the same for `m_tready`. `(0, 0)` means always-HIGH.

Three groups:
- **s_tvalid pattern only** — `m_tready` always HIGH; tests input throttling
- **m_tready pattern only** — `s_tvalid` always HIGH; tests output backpressure
- **Combined** — both signals vary simultaneously; tests worst-case interaction

Each stress DUT is a full `conv2d` instance with its own stimulus and checker processes. The checker waits for `mvalid AND mready` (the AXI4-Stream handshake) before consuming an expected value — so a spurious extra output would desynchronise the comparison and trigger a mismatch, and a missing output would leave the process waiting past the simulation time limit.

### 6. HTML generation (`--html`)
...
The sidebar also exposes a **Stress scenario** dropdown listing 30 additional timing
patterns. A stress scenario runs the same DUT configuration and golden vectors as a
base config, but introduces deliberate gaps in `s_tvalid` and/or `m_tready` to stress
the backpressure logic.

Selecting a stress scenario opens an overlay with:

- A plain-language explanation of what the scenario tests.
- An SVG digital waveform showing three rows: `s_tvalid` (green), `m_tready` (amber),
  and **data flows** (blue filled bars = both HIGH simultaneously — the only cycles
  when a pixel actually moves). Stall regions are shaded across all three rows.
- A 2×2 card grid explaining what each stall condition means and how the DUT should
  behave.
- A **View cycle trace →** button that closes the overlay and jumps to the base
  config's cycle-accurate trace.

A **← Back to base configs** button in the sticky header bar inside the overlay
closes it without navigating away.
...
### 3×3 kernel, 8-bit (testbench configuration)
...
BRAM inference confirmed: `u_line_buf` uses 1 RAMB36 (= 2 RAMB18), 0 LUT-as-RAM.
`u_win_buf`'s 72 registers are the 9-pixel × 8-bit FF shift register. No DSP48E1 used.

### 7×7 kernel, 8-bit, 1280×720, REPLICATE, FLUSH=false (comparison configuration)
...
**⚠ Bonded IOB exceeds device limit (409 / 200).** A 7×7 kernel has 49 output tap ports; each
carries TDATA(8) + TVALID + TREADY + TLAST + TUSER = 12 signals, giving >590 port pins before
adding the input and control pins. The XC7Z020 has 200 physical IOBs. This is expected — in
a real implementation the tap outputs would connect on-chip (to a MAC array or AXI interconnect)
and would not reach physical pins. The IOB count is informational only; simulation is unaffected.

**F7/F8 Muxes:** generated by the combinational REPLICATE clamping stage (`p_edge_out`), which
selects among 49 tap positions using cascaded LUT6+MUXF7+MUXF8 primitives.

**BRAM tiles (3):** 6 row buffer slots (KERN_ROWS−1) × 1,280 × 8 bits, packed 2 rows per RAMB36.

### Scaling

| Resource | Scales with |
|---|---|
| Slice LUTs | `KERN_ROWS × KERN_COLS` (output mux per tap) |
| Slice Registers | `KERN_ROWS × KERN_COLS × DATA_WIDTH` (tap pipeline registers) |
| F7/F8 Muxes | `KERN_ROWS × KERN_COLS` (REPLICATE mux tree) |
| Block RAM Tiles | `ceil((KERN_ROWS − 1) × LINE_WIDTH × DATA_WIDTH / 36,864)` |
| Bonded IOB | Exceeds 200-pin device limit for kernels larger than ~4×4 at 8-bit |
